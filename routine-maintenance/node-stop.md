...
- All three cluster settings controlling the individual drain periods have *different semantics*:
  - `server.shutdown.initial_wait`  is the minimum amount of time the drain step (1) can take
  - `server.shutdown.transactions.timeout`  is the maximum amount of time the drain step (2) can take
  - `server.shutdown.lease_transfer_iteration.timeout` mostly controls how frequently a progress message in printed in the message log during step (3). It should not be set to a value too low (e.g. lower than a few dozen milliseconds), for otherwise lease transfers will not occur at all and the drain will never complete. 
- Configuring `server.shutdown.initial_wait` and `server.shutdown.transactions.timeout` settings to `0s` would inevitably be disruptive to the user workload.
...
- avoid both excessively low and excessively high extremes; a reasonable setting is generally in a 5-10 minutes range. If it appears that a node requires a higher delay than 10 minutes to drain successfully, this may be an indication of inadequate cluster sizing or some other technical issue.
...
This setting should provide a sufficient time for all transactions in progress to complete. Therefore the `server.shutdown.transactions.timeout`  should be greater than any of the following:
...
Setting `server.shutdown.transactions.timeout` to a high value does not necessarily increase the elapsed time of a node drain because the `query_wait` is merely an upper bound: the node will proceed with the drain as soon as the last open transaction completes.



##### Cluster Setting: `server.shutdown.lease_transfer_iteration.timeout`
...
In practice, the transfer of ranges leaseholders in a cluster provisioned per Cockroach Labs sizing and configuration guidance, takes anywhere from tens of seconds to a small number of minutes. The default setting  for `server.shutdown.lease_transfer_iteration.timeout` works well in most cases. For large cluster nodes with high storage density (many ranges), this setting may be increased towards `20s` to reduce the message log chatter.

As a reminder, do not set `server.shutdown.lease_transfer_iteration.timeout` to a value lower than a few dozen milliseconds under any circumstances. This could cause the leases to fail to transfer altogether and cause the drain process to never complete.
